...
/**
 * @name requeue
 * @description requeue all failed message(s) based on specified
 *              criteria
 * @param {Object} [criteria] valid mongoose query criteria
 * @type {Function}
 * @return {Array[Message]} collection of requeued messages 
 * @public
 * @example
 *
 * //listen for message:queue:error events message queue
 ...
 * });
 *
 * //listen for message:queue events on message queue
 * Message._queue.on('message:requeue:success', function(message){
 *     ...
 *     //process success
 *     ...
 * });
...
MessageSchema.statics.requeue = function (criteria) {
  ...
  //obtain all unsent message(s) for requeue
  Message.unsent(criteria, function (error, unsents) {

    if (error) {
      //fire message:queue:error 
      Message._queue.emit('message:queue:error', error);
